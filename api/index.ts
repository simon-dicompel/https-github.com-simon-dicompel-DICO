...
const getPool = async () => {
    ...
    try {
        ...
        // Ensure Tables (Background)
        pool.request().query("IF COL_LENGTH('Orders', 'Notes') IS NULL ALTER TABLE Orders ADD Notes NVARCHAR(MAX)").catch(e => console.error("Setup error:", e.message));
...
app.get("/api/health", async (req, res) => {
    ...
    try {
        const activePool = await getPool();
        isConnected = activePool.connected;
    } catch (e) {
        isConnected = false;
    }
...
app.get("/api/users", async (req, res) => {
    try {
        const activePool = await getPool();
        const result = await activePool.request().query('SELECT * FROM usuarios ORDER BY nome');
...
app.delete("/api/users/:id", async (req, res) => {
    try {
        const activePool = await getPool();
        await activePool.request().input('id', sql.Int, parseInt(req.params.id)).query('DELETE FROM usuarios WHERE id = @id');
        res.status(204).end();
    } catch (err: any) {
        res.status(500).json({ error: err.message });
    }
...
const fetchOrdersWithItems = async (query: string, inputs: any[] = []) => {
    ...
    for (const order of orders) {
        const itemsRes = await activePool.request()
...
app.delete("/api/orders/:id", async (req, res) => {
    try {
        const activePool = await getPool();
        // Delete items first
        await activePool.request().input('id', sql.UniqueIdentifier, req.params.id).query('DELETE FROM OrderItems WHERE OrderID = @id');
        await activePool.request().input('id', sql.UniqueIdentifier, req.params.id).query('DELETE FROM Orders WHERE OrderID = @id');
        res.status(204).end();
    } catch (err: any) {
        res.status(500).json({ error: err.message });
    }
